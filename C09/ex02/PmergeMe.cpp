...
static T generateSequence(size_t size, T v)
{
	...
	// Generate the min jacobsthal sequence
	T seq;
	seq.push_back(0);
	seq.push_back(1);
...
template <typename T>
static void insertSort(T &v, size_t num, size_t left, size_t right)
{
	if (num < v[left])
...
std::vector<size_t> PmergeMe::sortVector(std::vector<size_t> v)
{
	...
	// Jacobian sequence (the order in which the elements of pendChain will be inserted)
	...
	for (size_t i = 1; i < jacobianSequence.size(); i++)
	{
		...
		if (index > pendChain.size() - 1)
			continue;
		insertSort(mainChain, pendChain[index], 0, inserted_count + index - 1);
		inserted_count++;
	}
	// 5. If the length of the original vector was odd, insert the last element
	if (v.size() % 2)
		insertSort(mainChain, v[v.size() - 1], 0, mainChain.size() - 1);
